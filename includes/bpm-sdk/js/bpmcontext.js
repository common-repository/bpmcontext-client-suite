...
if (typeof bpmcontext !== 'function'){
    ...
    var bpmcontext = function () {
        ...
        this.bpm_search_bar = function(){
            ...
            html_line = html_line.concat('&nbsp;<a onclick="bpmcontext.bpm_open_search_widget_run_search()" class="button" id="bpm_search_results_button">Search</a>');
            html_line = html_line.concat('</div>');
            html_line = html_line.concat('</div>');
                        ...
                        jQuery.each(bpm_params.home_page, function (index, value) {
                            ...
                            switch(index){
                                ...
                                case 'directory':
                                    ...
                                    if(bpm_dashboard[5]) home_page_section = home_page_section.concat('<div class="bpm_widget_box grid-item '+width_class+'">'+bpmcontext.bpm_make_homepage_widget( bpmcontext.bpm_make_directory(bpm_dashboard[5]),'bpm_employee_directory', 'Employee Directory')+'</div>');
                                    break;
                                case 'promoted':
                        ...
                        jQuery.getJSON('https://' + bpm_settings['bpm_server']  + '/'+bpm_settings['bpm_api']+'/bpmcontext_wordpress.php?callback=?', bpm_get_string + new_page_string, function (result) {
                            ...
                            if (result) {
                                ...
                                jQuery.each(result['SUBPAGES'], function (index, value) {
                                    ...
                                    html_string = html_string.concat('<div class="bpm-small-10 bpm-large-10 bpm-columns text-left">' + value['template_name'] + '</div>');
                                    if (value['this_index']) {
                                        html_string = html_string.concat('<div class="bpm-small-2 bpm-large-2 bpm-columns text-left"><input onClick="bpmcontext.bpm_add_to_sub_list(' + value['template_index'] + ')" type="checkbox" id="bpm_sub_select_' + value['template_index'] + '" value="bpm_sub_select_' + value['template_index'] + '" checked></div>');
                                    } else {
                                        html_string = html_string.concat('<div class="bpm-small-2 bpm-large-2 bpm-columns text-left"><input onClick="bpmcontext.bpm_add_to_sub_list(' + value['template_index'] + ')" type="checkbox" id="bpm_sub_select_' + value['template_index'] + '" value="bpm_sub_select_' + value['template_index'] + '" ></div>');
                                    }
                                    html_string = html_string.concat('</div>' + '<hr class="bpm_hr_gray">');
                                });
        ...
        this.add_custom_section = function(current_index){
            ...
            jQuery.each(bpm_params['custom_sections'], function (index, value) {
                if(value.position == current_index && (bpm_settings['TEMPLATELIBID'] == value.template_library_id || bpm_settings['TEMPLATE_ID'] == value.template_id) && (bpm_current_domain == value.domain || ! value.domain)) {
                    ...
                    return_data = '<ul class="accordion"  id="bpm_acc_' + value['id'] + '" data-accordion><li class="accordion-navigation"><a href="#acc_' + value['id'] + '" id="bpm_top_bar_' + value['id'] + '" class="bpm_top_bar bpm_nodecoration bpm_text_section"><span class="'+icon+'"></span>&nbsp;' + value.title + '<div style="float:right;display: inline;" ></div> ' + is_shared + '</a> <div id="acc_' + value['id'] + '" class="active bpm_custom_section bpm_section_height"> ' + bpmcontext.executeFunctionByName(value['callback'], window) + '</div> </li>  </ul>';
                }
            });
            return return_data;
        }
